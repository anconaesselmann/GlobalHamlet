...
public class DIFactory: DICProtocol {
    ...
    @objc public func build(idString: String) -> AnyObject! {
    ...
    @objc public func decorate(obj:InitArgsInterface, idString: String) {
        var args:[AnyObject] = getConstArgs(idString)
        if args.count > 0 {
            for (index, arg) in args.enumerate() {
                if let dict = arg as? [String:String] {
    ...
    @objc public func decorate(obj:InitArgsInterface) {
        let name = String(obj.dynamicType)
        let components = name.componentsSeparatedByString(".")
        decorate(obj, idString: components.last!)
    }
    @objc public func get(idString: String) -> AnyObject! {
        if _dict != nil {
